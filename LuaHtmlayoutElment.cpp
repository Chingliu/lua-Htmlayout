...
Lunar<CLuaHtmlayoutElement>::RegType CLuaHtmlayoutElement::methods[] =
{
	...
	method(CLuaHtmlayoutElement, set_html_utf8),
	method(CLuaHtmlayoutElement, get_html),
	method(CLuaHtmlayoutElement, get_html_utf8),
	...
	method(CLuaHtmlayoutElement, xcall),

	method(CLuaHtmlayoutElement, http_request),
...
int CLuaHtmlayoutElement::set_html_utf8(lua_State* L)
{		
	m_domelement.set_html((const unsigned char*)PAS(L, 1), PASL(L, 1), PAI(L, 2));
	return 0;
}
...
int CLuaHtmlayoutElement::get_html_utf8(lua_State* L)
{	
	json::value v = m_domelement.get_html(lua_toboolean(L, 1));
	aux::bytes  b = v.get_bytes();
	lua_pushlstring(L, (const char*)b.start, b.length);
	return 1;
}
...
int CLuaHtmlayoutElement::xcall(lua_State* L)
{
	...
	json::value v = m_domelement.xcall(PAS(L, 1), 
		vlist[0],vlist[1],vlist[2],vlist[3],vlist[4],
		vlist[5],vlist[6]);
	lua_pushstring(L, aux::w2a(v.to_string().c_str()));
	return 1;
}

int CLuaHtmlayoutElement::http_request(lua_State* L)
{
	lua_pushboolean(L, HLDOM_OK == ::HTMLayoutHttpRequest(m_domelement, aux::a2w(PAS(L, 1)), PAI(L, 2),GET_ASYNC, NULL, 0));
	return 1;
}