...
//utf-8转换成ansi编码
static int lua_utf2a(lua_State* L)
{
	lua_pushstring(L, w2t(u2w(PAS(L, 1))));
	return 1;
}
...
int mfc_init(lua_State* L)
{
	...
	if(lua_isstring(L, 1))
	{
		...
		lua_pushstring(L, PAS(L, 1));
		lua_setglobal(L, "mfc_thread_name");
	}
	return 0;
}
...
{
	CWnd* pWnd =  new CHtmlayoutWDlg(L, pszUrl, pParent, TRUE, 9103);
	if(pWnd)
...
int   DestroyHtmlayoutControl(CWnd* pControl)
{
	if(pControl == NULL)
		return -1;
	pControl->SendMessage(WM_CLOSE);
	return 0;
}

int   SendMessage2Control(CWnd* pControl,  const char* pszName,   const char* pszMsg, int nLen)
{
	if(pControl == NULL)
		return -1;
	
	MsgData name(pszName);
	MsgData md(pszMsg, nLen);
	pControl->SendMessage(WM_HTMLAYOUTCTRL, (WPARAM)&name, (LPARAM)&md);
	return 0;
}
...
int   CallControlFunc(CWnd* pControl, lua_State* L, const char* pszFuncName, int nParams)
{
	...
	CHtmlayoutWDlg* pCtrl = (CHtmlayoutWDlg *)pControl;
	if(pCtrl->m_L != L)
	{
		ASSERT(0);
		return -2;
	}
	return pCtrl->CallViewFunction(pszFuncName, nParams);
}
