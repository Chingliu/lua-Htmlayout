...
local function LoadModuleScript(mn, txt, name)
	...
	if txt == nil then
		local txt = string.format(load_module_txt, mn);
		loadstring(txt, name)();
		return;
	end
...
--统一处理页面消息的分发
function HandleHtmlayoutEvent(he, target, event_type, reason, issinking)
	...
	if type(he['onEventHook']) == 'function' then
		he['onEventHook'](he, event_name, target, event_type, reason, issinking);
	end

	if he[event_name] and type(he[event_name]) == 'function' then
		he[event_name](he, target, event_type, reason, issinking);
		return true;
	end
	...
	if type(he['onEvent']) == 'function' then
		return he['onEvent'](he, event_name, target, event_type, reason, issinking);
	end
	return false;
end
...
function onHtmlayoutDocumentComplete(view, hroot, ...)
	view = ui.view.CreateView(view, true);
...
--窗口关闭时的处理
function onWindowClose(hroot, view)	
	ui.view.DeleteView(view);
