...
BEGIN_EVENTSINK_MAP(SimpleBrowser,CWnd)
	...
	ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
			 273,//DISPID_NEWWINDOW3
			 _OnNewWindow3, 
			 VTS_PDISPATCH VTS_PBOOL VTS_I4 VTS_BSTR VTS_BSTR)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
		...
		if      (post_data[offset] == _T('%')) {
			...
			if ((offset + 2) < post_data.GetLength()) {
				...
				if ((digit1 >= 0) && (digit2 >= 0)) {
					...
					if (parsing_name) 
						names[size - 1] += character;
					else              
						values[size - 1] += character;
			...
			if (parsing_name) {
				...
				values[size - 1] += (post_data[offset]);
			}
			...
			if (parsing_name) names[size - 1] += (post_data[offset]);
			else              values[size - 1] += (post_data[offset]);
